...
from .once.once_semi_dataset import (
    ONCEPretrainDataset,
    ONCELabeledDataset,
    ONCEUnlabeledDataset,
    ONCETestDataset,
    ONCEUnlabeledPairDataset,
    split_once_semi_data,
)
from .once.once_dataset_pretrain import ONCEDatasetPretrain_ADPT
from .nuscenes.nuscenes_semi_dataset import (
    NuScenesPretrainDataset,
    NuScenesLabeledDataset,
    NuScenesUnlabeledDataset,
    NuScenesTestDataset,
    split_nuscenes_semi_data,
)
from .kitti.kitti_semi_dataset import (
    KittiPretrainDataset,
    KittiLabeledDataset,
    KittiUnlabeledDataset,
    KittiTestDataset,
    split_kitti_semi_data,
)
...
def build_semi_dataloader(
    ...
    train_infos, test_infos, labeled_infos, unlabeled_infos = _semi_dataset_dict[
        dataset_cfg.DATASET
    ]["PARTITION_FUNC"](
        dataset_cfg=dataset_cfg,
    ...
    if dist:
        pretrain_sampler = torch.utils.data.distributed.DistributedSampler(
            pretrain_dataset
        )
    else:
    ...
    pretrain_dataloader = DataLoader(
        pretrain_dataset,
        batch_size=batch_size["pretrain"],
        pin_memory=True,
        num_workers=workers,
        shuffle=(pretrain_sampler is None) and True,
        collate_fn=pretrain_dataset.collate_batch,
        drop_last=False,
    ...
    labeled_dataloader = DataLoader(
        ...
        pin_memory=True,
        num_workers=workers,
        shuffle=(labeled_sampler is None) and True,
        collate_fn=labeled_dataset.collate_batch,
        drop_last=False,
    ...
    unlabeled_dataloader = DataLoader(
        ...
        pin_memory=True,
        num_workers=workers,
        shuffle=(unlabeled_sampler is None) and True,
        collate_fn=unlabeled_dataset.collate_batch,
        drop_last=False,
...
def build_unsupervised_dataloader(
    dataset_cfg,
    class_names,
    batch_size,
    dist,
    ...
    datasets = {"unlabeled": unlabeled_dataset, "test": test_dataset}
    dataloaders = {"unlabeled": unlabeled_dataloader, "test": test_dataloader}
    samplers = {"unlabeled": unlabeled_sampler, "test": test_sampler}

    return datasets, dataloaders, samplers


def build_dataloader_multi_db(
    dataset_cfg_1,
    dataset_cfg_2,
    class_names_1,
    class_names_2,
    ...
    if dist:
        if training:
            ...
            sampler = DistributedSampler(
                concat_dataset, world_size, rank, shuffle=False
            )
    else:
        sampler = None
    dataloader = DataLoader(
        ...
        num_workers=workers,
        shuffle=(sampler is None) and training,
        collate_fn=dataset_1.collate_batch,
        drop_last=False,
        sampler=sampler,
        timeout=0,
    )
