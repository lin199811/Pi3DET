...
class M3ED_SEQ:
    ...
    def load_seq(self):
        ...
        # for track
        sequence_name, seq_name_to_len, sample_idx_list = m3ed_utils.group_samples(
            self.sweeps_num, self.sequence_name, 50
        )  # 5s
            ...
            if self.dataset_cfg.get("VIRTUAL_POSE", False):
                self.infos.append(
                    dict(
                        ...
                        pose=(
                            self.get_virtual_pose(idx, to_world=True)
                            if self.platform == "Spot"
                            else self.get_virtual_pose(idx, drone=True, to_world=True)
                        ),
                    )
                )
            else:
    ...
    def get_lidar(self, idx, virtual=False):
        ...
        packets = client.Packets(
            [client.LidarPacket(opacket, self.metadata) for opacket in ouster_sweep],
            self.metadata,
        )
    ...
    def get_sequence_data(self, points, sample_idx, max_sweeps):
        ...
        sample_idx_pre_list = np.clip(
            sample_idx + np.arange(-int(used_num_sweeps - 1), 0), 0, 0x7FFFFFFF
        )
        ...
        for sample_idx_pre in sample_idx_pre_list:
            ...
            points_pre_global = np.dot(
                expand_points_pre, m3ed_utils.transform_inv(pose_pre).T
            )[:, :3]
    ...
    def get_virtual_pose(self, index, drone=False, to_world=False, label=False):
        ...
        if drone:
            ...
            virtual_pose = virtual_translate @ virtual_pose
        if to_world:
            vir_T_L0 = Ln_T_L0 @ m3ed_utils.transform_inv(
                virtual_pose
            )  # L0_T_Ln @ Ln_T_vir
            return vir_T_L0
    ...
    def get_fov_flag(self, points, mask=True, return_depth=False, downsample=1):
        ...
        pts_img, _ = cv2.projectPoints(
            points_cam[:, :3].astype(np.float32), rvecs, tvecs, self.K, self.D
        )
        imgpts = pts_img[:, 0, :]
        depth = points_cam[:, 2]
        ...
        if mask:
            ...
            kept1 = (
                (imgpts[:, 1] >= 0)
                & (imgpts[:, 1] < self.image_shape[1])
                & (imgpts[:, 0] >= 0)
                & (imgpts[:, 0] < self.image_shape[0])
                & (depth > 0.0)
            )
            if not return_depth:
                return kept1
                ...
                sort = np.argsort((ranks + 1 - depth / 100.0))
    ...
    def convert_boxes_from_vir_to_n(self, index, boxes, drone=False, label=False):
        ...
        expand_centroids = np.concatenate(
            [boxes[:, :3], np.ones((boxes.shape[0], 1))], axis=-1
        )
    ...
    def get_anno_info(self, index):
        ...
        gt_names = np.array(self.ori_class_names)[
            np.abs(gt_boxes_info[:, -2].astype(np.int32)) - 1
        ]
        ...
        if self.platform != "Car":
            if self.platform == "Spot":
                ...
                drone = True
            if self.sequence_name != "srt_under_bridge_2":
                gt_boxes_lidar = self.convert_boxes_from_vir_to_n(
                    index, gt_boxes_lidar, drone, label=True
                )
            else:
                gt_boxes_lidar[:, 2] += 1.3

        anno_info = dict({"gt_names": gt_names, "gt_boxes": gt_boxes_lidar})
        return anno_info
    ...
    def __getitem__(self, index):
        ...
        points, _ = self.get_sequence_data(
            points, frame_index, max_sweeps=self.dataset_cfg.MAX_SWEEPS
        )
        ...
        input_dict.update(
            {"gt_names": anno_dict["gt_names"], "gt_boxes": anno_dict["gt_boxes"]}
        )
        ...
        if self.dataset_cfg.get("VIRTUAL_POSE", False):
            ...
            if self.platform == "Falcon":
                ...
                box_lidar = self.convert_boxes_from_n_to_vir(
                    index, box_lidar, drone=True
                )
...
class OFFM3EDDatasetSeqs(DatasetTemplate):
    ...
    def __init__(
        ...
        sequence_paths = [Path(self.root_path) / seq_path for seq_path in sequences]
        self.ori_class_names = ["Vehicle", "Pedestrian", "Pedestrian"]
    ...
    def include_m3ed_sequence_data(self, sequence_paths):
        ...
        for i in range(self.seq_num):
            setattr(
                self,
                f"seq_{i}",
                M3ED_SEQ(sequence_paths[i], self.dataset_cfg, self.class_names),
            )
            self.infos.extend(getattr(getattr(self, f"seq_{i}"), "infos"))
            self.seq_name_to_len.update(
                getattr(getattr(self, f"seq_{i}"), "seq_name_to_len")
            )
        seq_frame_num = [
            getattr(getattr(self, f"seq_{i}"), "sweeps_num")
            for i in range(self.seq_num)
        ]
        frame_info = []
        ...
        for i, num in enumerate(seq_frame_num):
            frame_info.extend([f"{i}_{frame_id}" for frame_id in range(num)])
    ...
    def get_multi_frame_lidar(self, idx, frame_nums=10):
        ...
        seq_idx, frame_idx = [eval(fac) for fac in single_frame_info.split("_")]
        points = getattr(self, f"seq_{seq_idx}").get_lidar(frame_idx)
        points, _ = getattr(self, f"seq_{seq_idx}").get_sequence_data(
            points, frame_idx, max_sweeps=frame_nums
        )
        return points
    ...
    def kitti_eval(self, eval_det_annos, eval_gt_annos, class_names):
        ...
        map_name_to_kitti = {"car": "Car", "Vehicle": "Car", "Pedestrian": "Pedestrian"}
        ...
        for frame_index in range(len(self.frame_info)):
            ...
            if self.dataset_cfg.get("VIRTUAL_POSE", False):
                ...
                if self.platform == "Falcon":
                    gt_boxes_3d = getattr(
                        self, f"seq_{seq_idx}"
                    ).convert_boxes_from_n_to_vir(frame_idx, gt_boxes_3d, drone=True)
                else:
                    gt_boxes_3d = getattr(
                        self, f"seq_{seq_idx}"
                    ).convert_boxes_from_n_to_vir(frame_idx, gt_boxes_3d)

            single_anno_info.update(
                {"gt_names": gt_names, "gt_boxes": gt_boxes_3d[:, :7]}
            )
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            aug_names = [
                aug.NAME for aug in self.dataset_cfg.DATA_AUGMENTOR.AUG_CONFIG_LIST
            ]
            if "occ_gt_sampling" in aug_names:
                free_logits_file_path = (
                    self.root_path
                    / "free_space_logits"
                    / "training"
                    / ("%s.npy" % input_dict["frame_id"])
                )
